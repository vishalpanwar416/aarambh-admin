...
///
/// The source of truth is the `role: 'admin'` **custom claim** on the Firebase
/// Auth ID token. Claims are settable only via the Admin SDK, so a user cannot
/// grant the role to themselves.
///
...
/// Resolve `user`'s role from its ID token and cache it. Pass null on sign-out.
///
...
export async function refreshAdminClaim(user: User | null, forceRefresh = false): Promise<boolean> {
  ...
  try {
    ...
    currentUserIsAdmin = token.claims?.role === ADMIN_ROLE;
  } catch {
...
/// Whether the signed-in user is an admin. Reflects the last `refreshAdminClaim`.
export const isCurrentUserAdmin = () => currentUserIsAdmin;
